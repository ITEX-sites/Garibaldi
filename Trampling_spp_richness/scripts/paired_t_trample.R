...
t.test(Freq ~ Treat, data = mydata, paired = TRUE) # this is two-tailed; p=

t.test(Freq ~ Treat, data = mydata, paired = TRUE, alternative = "less") # this is one tailed and is more appropriate to our hypothesis
# t.test(Freq ~ Treat, data = mydata, paired = TRUE, alternative = "greater") # this is one tailed but in the wrong direction - don't use

# log values paired t
#### Paired t test. The data must be ordered so that the first observation of Group 1
#is the same subject as the first observation of Group 2
t.test(Log ~ Treat, data = mydata, paired = TRUE) # this is two-tailed; p=

t.test(Log ~ Treat, data = mydata, paired = TRUE, alternative = "less") # this is one tailed and is more appropriate to our hypothesis
